...
1. `cargo release --package teloxide-core-ng [major|minor|patch]` (if your git remote isn't named `origin`, you need to specify it with `--push-remote <remote_name>`)
...
3. Repeat 1 and 2 with the `teloxide-macros-ng` crate if there were any changes
4. Make sure all dependencies in `crates/teloxide-ng/Cargo.toml` point to the released versions from crates.io, including `teloxide-core-ng` and `teloxide-macros-ng`. Commit the change if this isn't the case
