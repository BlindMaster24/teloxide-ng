...
## Crate Purpose
`teloxide-ng` is the high-level framework crate used by bot developers. It should remain ergonomic, stable, and well-documented for end users.
...
- API wiring requires changes in `teloxide-core-ng` or `teloxide-macros-ng`
...
- `cargo fmt --all`
- `cargo clippy -p teloxide-ng --all-targets --features "full nightly"`
- `cargo test -p teloxide-ng --features "full nightly"`
- `cargo check -p teloxide-ng --examples --features full`
...
- SQLite:
  - `cargo test -p teloxide-ng --test sqlite --features "sqlite-storage-nativetls cbor-serializer bincode-serializer"`
- Postgres:
  - `cargo test -p teloxide-ng --test postgres --features "postgres-storage-nativetls cbor-serializer bincode-serializer"`
