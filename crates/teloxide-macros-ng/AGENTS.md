...
## Crate Purpose
`teloxide-macros-ng` provides procedural macros used by `teloxide-ng`, primarily command-related derives and attribute handling.
...
4. integration confidence via `crates/teloxide-ng/tests/command.rs`
...
- `cargo fmt --all`
- `cargo clippy -p teloxide-macros-ng --all-targets`
- `cargo test -p teloxide-macros-ng`
...
- Validate end-user behavior in `crates/teloxide-ng/tests/command.rs`
...
- `crates/teloxide-macros-ng/AGENTS.md`
- root `AGENTS.md`
in the same PR.
