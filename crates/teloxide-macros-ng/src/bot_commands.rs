...
pub(crate) fn bot_commands_impl(input: DeriveInput) -> Result<TokenStream> {
    ...
    let trait_impl = quote! {
        impl teloxide_ng::utils::command::BotCommands for #type_name {
            #fn_descriptions
            #fn_parse
            #fn_commands
        }
...
fn impl_commands(infos: &[Command]) -> proc_macro2::TokenStream {
    ...
    quote! {
        fn bot_commands() -> ::std::vec::Vec<teloxide_ng::types::BotCommand> {
            use teloxide_ng::types::BotCommand;
            ::std::vec![#(#commands),*]
        }
    }
}
...
fn impl_descriptions(infos: &[Command], global: &CommandEnum) -> proc_macro2::TokenStream {
    ...
    quote! {
        fn descriptions() -> teloxide_ng::utils::command::CommandDescriptions<'static> {
            use teloxide_ng::utils::command::{CommandDescriptions, CommandDescription};
            use std::borrow::Cow;
...
fn impl_parse(
    ...
    quote! {
         fn parse(s: &str, bot_name: &str) -> ::std::result::Result<Self, teloxide_ng::utils::command::ParseError> {
