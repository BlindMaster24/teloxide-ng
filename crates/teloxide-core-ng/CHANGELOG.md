...
- Previously deprecated items ([#1013][pr1013])
  ...
  `teloxide_ng::dispatching::{update_listeners, repls}` (use `reloxide::{update_listeners, repls}` instead)
...
- Move core code here from the [`teloxide`] main repo, for older changes see it's [`CHANGELOG.md`].
  - Following modules were moved:
    ...
    - `errors`
    - `net` [private]
  - `client_from_env` was moved from `teloxide_ng::utils` to crate root of `teloxide-core`
...
- Refactor file downloading ([#30][pr30]):
  ...
  - Change return type of `download_file_stream` â€” return ` Stream<Result<Bytes>>``, instead of  `Future<Result<Stream<Result<Bytes>>>>``
