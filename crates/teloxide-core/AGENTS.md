...
## Crate Purpose
`teloxide-core` is the low-level Telegram Bot API client crate. It owns protocol models, method payloads, request abstractions, network layer, and adaptors.
...
Change this crate first for Bot API protocol updates. Touch `teloxide` only when high-level exposure/wiring is required.
...
- `cargo fmt --all`
- `cargo clippy -p teloxide-core --all-targets --features "full nightly"`
- `cargo test -p teloxide-core --features "full nightly"`
- `cargo test -p teloxide-core --doc --features "full nightly"`
- `cargo check -p teloxide-core --examples --features "full nightly"`
- `just ci`
...
- `crates/teloxide-core/AGENTS.md`
- root `AGENTS.md`
in the same PR.
