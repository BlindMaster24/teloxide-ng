...
When you introduce changes that bump suppported Telegram Bot API version (e.g. 6.9 â†’ 7.0), you must:

- Specify your changes in [crates/teloxide-core-ng/CHANGELOG.md](crates/teloxide-core-ng/CHANGELOG.md) file
...
- Change TBA version and it's announce date in `api_version: ApiVersion(ver: "7.0", date: "December 29, 2023"),` line in head of [crates/teloxide-core-ng/schema.ron](crates/teloxide-core-ng/schema.ron) file
- Change TBA version in `(Currently, version â€¦ is supported)` line in head of [crates/teloxide-core-ng/src/lib.rs](crates/teloxide-core-ng/src/lib.rs) file
- Change TBA version in `Currently, version â€¦ of` line in head of [crates/teloxide-ng/src/lib.rs](crates/teloxide-ng/src/lib.rs) file
- Change TBA version in `â€¦https://img.shields.io/badge/API%20coverageâ€¦` line in [crates/teloxide-core-ng/README.md](crates/teloxide-core-ng/README.md) file
- Change TBA version in `â€¦https://img.shields.io/badge/API%20coverageâ€¦` line in [README.md](README.md) file

### Adding a new TBA method

#### Step 1:

Add the method and its info to `crates/teloxide-core-ng/src/schema.ron` file.
...
Rerun the tests, and look at the compiler errors. Most likely there will be errors from `crates/teloxide-core-ng/src/adaptors`, `crates/teloxide-core-ng/src/requests/requester.rs` and `crates/teloxide-core-ng/src/bot/api.rs`.
...
3. Sometimes you will encounter situation when it's impossible to write something in `schema.ron`. It may be some doc comment that renders incorrectly or some type (like `Option<DateTime<Utc>>`) impossible to write. In such cases you can workaround it with `crates/teloxide-core-ng/src/codegen/patch.rs` file.

4. In very rare cases, autogenerated files in `crates/teloxide-core-ng/src/payloads/*` may have errors related to deriving impls for the `PartialEq`, `Eq`, or `Hash` traits. In such cases, remove them manually and add the payload as an exception to `codegen_payloads()` in `crates/teloxide-core-ng/src/payloads/codegen.rs`.

#### Other notes

1. If you mess up the .ron and run the codegen, it is better to reset the files, rather than to try and fix it all by hand:
`git restore crates/teloxide-core-ng/src/local_macros.rs crates/teloxide-core-ng/src/payloads.rs crates/teloxide-core-ng/src/payloads/ crates/teloxide-core-ng/src/requests/requester.rs && git clean -fd crates/teloxide-core-ng/src/payloads/`
